...
with pd.ExcelWriter(out, engine="openpyxl") as writer:
    ...
    if end_row >= start_row and end_col >= start_col:
        ...
        exclude_headers = {"GB%", "FB%", "GB %", "FB %"}
        for col_idx, col_name in enumerate(df_season.columns, start=1):
            if col_idx < start_col:
                continue
            name_norm = str(col_name).strip()
            name_up = name_norm.upper()
            name_lo = name_norm.lower()

            # Skip heatmap for GB% / FB% (or any header that clearly matches those)
            if name_norm in exclude_headers or name_up in {"GB%", "FB%"}:
                continue
            if "%" in name_norm and (name_lo.startswith("gb") or name_lo.startswith("fb")):
                continue
    ...
    # Coach Notes column: make font size 12 (if present)
    try:
        note_col_idx = None
        for i, name in enumerate(df_season.columns, start=1):
            n = str(name).strip().lower()
            if "coach" in n and "note" in n:
                note_col_idx = i
                break

        if note_col_idx:
            note_letter = get_column_letter(note_col_idx)
            note_font = Font(size=12)
            note_align = Alignment(horizontal="left", vertical="top", wrap_text=True)
            for r in range(2, ws.max_row + 1):
                c = ws[f"{note_letter}{r}"]
                c.font = note_font
                c.alignment = note_align
            # a little wider for notes
            ws.column_dimensions[note_letter].width = max(ws.column_dimensions[note_letter].width or 12, 28)
    except Exception:
        pass
