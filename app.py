...
# ------------------------------
# ACCESS + TERMS UI STYLES
# ------------------------------
st.markdown("""
<style>
.rp-auth-wrap { max-width: 520px; margin: 0 auto; padding-top: 2.5rem; }
.rp-auth-card {
  border-radius: 18px;
...
.rp-auth-title { font-size: 1.55rem; font-weight: 900; text-align: center; }
.rp-auth-sub { text-align: center; opacity: 0.85; margin-bottom: 1.1rem; font-weight: 700; }
.rp-auth-mini { font-size: 0.85rem; opacity: 0.75; line-height: 1.35; margin-top: 0.6rem; }
.rp-pill {
...
.rp-auth-footer { text-align: center; font-size: 0.8rem; opacity: 0.6; margin-top: 1rem; }
</style>
...
def require_team_access():
    ...
    # allow viewing Terms before login
    if "show_terms_only" not in st.session_state:
        st.session_state.show_terms_only = False
    ...
    # -----------------------------
    # Login screen (spruced up)
    # -----------------------------
    st.markdown('<div class="rp-auth-wrap">', unsafe_allow_html=True)
    st.markdown(
        """
        <div class="rp-auth-card">
            <div class="rp-auth-title">RP Spray Analytics</div>
            <div class="rp-auth-sub">Enter your team access code to continue</div>
            <span class="rp-pill">Team-isolated data</span>
            <span class="rp-pill">Coach-only access</span>
            <span class="rp-pill">Season reports</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    c1, c2 = st.columns(2)
    with c1:
        submit = st.button("Unlock App", use_container_width=True)
    with c2:
        view_terms = st.button("View Terms", use_container_width=True)

    st.markdown(
        """
        <div class="rp-auth-mini">
            Codes are case-sensitive. Do not share access codes outside your program.
        </div>
        <div class="rp-auth-footer">
            © 2026 RP Spray Analytics • Unauthorized access or sharing is prohibited
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    if view_terms:
        st.session_state.show_terms_only = True
        st.rerun()
...
def _render_terms_gate(allow_back: bool):
    st.markdown('<div class="rp-auth-wrap">', unsafe_allow_html=True)
    st.markdown('<div class="rp-auth-card">', unsafe_allow_html=True)

    st.markdown("### Terms of Use – RP Spray Analytics")
    st.markdown(
        """
**1) Authorized Use**
- Licensed for authorized coaching staff and approved program personnel only.
- Access codes are confidential and may not be shared outside your program.

**2) Restrictions**
You may not:
- Share, sell, sublicense, or redistribute this application or access codes.
- Copy report formats, workflows, parsing rules, or exports for competitive or commercial use.
- **Reverse engineer, decompile, disassemble, or replicate application logic for competitive or commercial purposes.**

**3) Data Responsibility**
- Users are responsible for uploaded data and resulting decisions.
- Analytics are informational and do not guarantee outcomes.

**4) Enforcement**
- Access may be revoked immediately for violations or suspected unauthorized use.
        """
    )
    ...
    c1, c2 = st.columns(2)
    with c1:
        back = st.button("Back", use_container_width=True) if allow_back else False
    with c2:
    ...
    return agree, cont, back

# Terms page can be viewed from login (before access code)
if st.session_state.get("show_terms_only", False):
    agree, cont, back = _render_terms_gate(allow_back=True)
...
# Normal one-time gate (after login)
if not st.session_state.terms_accepted:
    agree, cont, _ = _render_terms_gate(allow_back=False)
